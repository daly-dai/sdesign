...
通过工厂函数 `createDrawer` 创建自动管理生命周期的 Drawer 容器组件（`SDrawerContainer` 为别名）。

### 核心特性

- **自动状态管理**：通过 ref 暴露 `open(params)` / `close()` 方法，无需手动维护 visible 状态
- **自动卸载**：抽屉关闭时内部组件完全卸载，所有 useState 等自动销毁，无需手动重置
...
| 参数    | 描述         | 类型                                 | 默认值 |
| ------- | ------------ | ------------------------------------ | ------ |
| Content | 抽屉内容组件 | `ComponentType<DrawerChildProps<P>>` | -      |
| options | 配置项       | `CreateDrawerOptions`                | `{}`   |
...
| 方法  | 描述                             | 类型                  |
| ----- | -------------------------------- | --------------------- |
| open  | 打开抽屉并传参                   | `(params: P) => void` |
| close | 手动关闭抽屉（触发 beforeClose） | `() => void`          |
...
| ---------- | ------------------------------------------ | ------------ | ------ |
| params     | open() 传入的参数                          | `P`          | -      |
| open       | 抽屉是否可见（绑定到 antd Drawer 的 open） | `boolean`    | -      |
| onClose    | 关闭抽屉（触发 beforeClose 守卫）          | `() => void` | -      |
| onSuccess  | 操作成功后关闭抽屉并触发回调               | `() => void` | -      |
