...
**UseProTableOptions** extends Omit<
Options<any, any>,
...
- defaultParams?: Record<string, unknown> — 表单初始值，mount 时写入表单。 适用场景：详情页返回时恢复上次的查询条件。 注意：此值仅在 mount 时生效一次，不会跟随外部变化自动更新。
- ready?: boolean — 是否准备好，默认 true。 false 时跳过初始化请求；变为 true 后自动触发搜索（仅首次）。 常用于等待字典接口等依赖返回后再请求列表。
- manual?: boolean — 是否手动触发首次请求，默认 false
- paginationFields?: ProPaginationFields — 分页字段映射
- refreshDeps?: any[] — 依赖刷新数组。当数组内任意值变化时，自动重置到第一页并刷新。 适用场景：外部 tab / 筛选条件变化后自动更新列表。
