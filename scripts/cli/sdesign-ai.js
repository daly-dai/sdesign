 ...
 *
 * 核心思路：渐进式文档结构 — README.md 索引 + ai/components/ 详细文档
 *
 * 用法:
...
function main() {
  ...
  // 核心复制 README.md（索引） + components/（详细文档）
  copyTo('README.md', path.join(root, 'README.md'));
  copyComponentsDir(path.join(root, 'ai', 'components'));

  if (all || flags.has('--cursor')) {
    copyTo('README.md', path.join(root, '.cursorrules'));
  }
  if (all || flags.has('--claude')) {
    copyTo('README.md', path.join(root, 'CLAUDE.md'));
  }
  if (all || flags.has('--copilot')) {
    copyTo('README.md', path.join(root, '.github', 'copilot-instructions.md'));
  }

  console.log(
    '\n✅ 完成！ 渐进式文档结构：README.md（索引）+ ai/components/（按需读取）。',
  );
